...
# Keep the system ncurses tools ahead of Vast's /venv/main tools.  This is
# important for TERM=alacritty and TERM=xterm-kitty over SSH/tmux.
ENV PATH="/root/.cargo/bin:/root/.kilo/bin:/opt/moa-python/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:${PATH}"
...
# cargo-binstall plus the MOA/Furiosa tools used by the official baseline.
...
# Small, isolated CPU Python environment for scripts/generate_references.py.
RUN /usr/bin/python3 -m venv /opt/moa-python \
    && /opt/moa-python/bin/python -m pip install --no-cache-dir --upgrade pip \
    && /opt/moa-python/bin/python -m pip install --no-cache-dir numpy safetensors \
    && /opt/moa-python/bin/python -m pip install --no-cache-dir torch --index-url https://download.pytorch.org/whl/cpu
...
COPY docker/moa-zsh.zsh /etc/moa/zsh.zsh
COPY docker/kilo.jsonc /root/.config/kilo/kilo.jsonc

RUN touch /etc/tmux.conf \
    && printf '\nsource-file /etc/moa/tmux.conf\n' >> /etc/tmux.conf \
    && printf '\nsource /etc/moa/zsh.zsh\n' >> /etc/zsh/zshrc \
    && chsh -s /usr/bin/zsh root

# Build-time smoke checks.  Use the system ncurses binary explicitly so the
# image fails immediately if either SSH terminal type is unavailable.
RUN test "$(command -v infocmp)" = "/usr/bin/infocmp" \
    ...
    && /opt/moa-python/bin/python -c 'import numpy, torch, safetensors; print("python refs: OK", numpy.__version__, torch.__version__, safetensors.__version__)' \
    && rustc --version \
    && cargo --version \
    && tmux -V \
    && kilo --version
